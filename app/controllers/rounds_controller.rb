class RoundsController < ApplicationController
  ...
  # GET /rounds.xml
  def index
    @rounds = Round.find(:all, :order => 'id DESC')
  ...
  def show
    ...
    @entries = Entry.find(:all, :conditions => ["round_id = ?", @round], :order => ["created_at DESC"])
    @users = User.find(:all)
    @waiting = @users.delete_if {|x| @round.users.include?(x) }
